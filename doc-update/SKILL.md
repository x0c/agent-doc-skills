...
- **Run the counterfactual on every detour you just replayed: assume the docs had already been perfect** — the recipe, class name, path, or config you discovered the hard way is already written down exactly right. Re-walk the detour under that assumption: if the workarounds themselves would still have to be executed (multi-step gymnastics, in-memory toggles, guess-and-check against a binary, connecting somewhere undocumented), that residual is **environment debt, not knowledge debt**. Perfect docs cannot remove it — only changing the environment can (a config option, a profile, a self-verifying recipe, a doc-fix that lives closer to the source). Record it as an improvement item alongside the pitfall entry: the pitfall entry teaches the next Agent to detour *correctly*; the improvement item is what makes the detour *unnecessary*. Hard signal — if this session had to decompile, unzip, or reverse-engineer a binary/dependency just to complete verification, that is always environment debt; the required knowledge was not reachable by design, and it will not be next time either.
...
- **Environment friction that survives perfect docs** — detours whose residual cost remains even after the pitfall is fully documented (see the counterfactual in the recall section). The workaround gets recorded as a pitfall; the removal of the workaround gets recorded as an improvement item. The two are different deliverables: one routes around the friction, the other retires it. When unsure whether a friction is worth an improvement item, rank by how many retry rounds the detour took this session — that count is available from memory alone.
...
| **Environment / tooling friction that perfect docs cannot remove** | The project's backlog or todo ledger (where the project tracks pending work); if none exists, a `docs/troubleshooting/` or `docs/design/` entry the root `AGENTS.md` can point to | "Local smoke requires in-memory permission toggles → add a smoke profile"; "recipe references a class name inside a binary → make it self-verifying" |
