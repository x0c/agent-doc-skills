...
7. Root `AGENTS.md` document navigation contains concise content summaries and one shared instruction to read relevant documents. Docs under `docs/` describe their scope and contents without duplicating the navigation instruction.
...
- Each item records the planned document name, entry anchor, and a concise summary of its intended content.
